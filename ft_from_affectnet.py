...
parser.add_argument('--arch', '-a', metavar='ARCH', default='IR_152', choices=model_names,
...
parser.add_argument('-b_t', '--batch-size_t', default=16, type=int,
                    metavar='N', help='mini-batch size (default: 256)')
...
parser.add_argument('--model_dir','-m', default='./ir_152_from_face_224_align_models', type=str)
...
def main():
    ...
    train_transform = transforms.Compose([ # refer to https://pytorch.org/docs/stable/torchvision/transforms.html for more build-in online data augmentation
        transforms.Resize([int(256 * INPUT_SIZE[0] / 112), int(256 * INPUT_SIZE[0] / 112)]), # smaller side resized
    ...
    if args.pretrained:
        ...
        checkpoint = torch.load('Backbone_IR_152_Epoch_112_Batch_2547328_Time_2019-07-13-02-59_checkpoint.pth')
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        average_f1 = 0
        for j in range(0,23):
            average_f1 += f1_score(pred_score[:,j].cpu().detach().numpy(), target_var[:,j].cpu().detach().numpy(), average='macro')
        average_f1 = average_f1/23


        # prec1, prec5 = accuracy(pred_score.data, target, topk=(1, 5))
        losses.update(loss.item(), input.size(0))
        top1.update(mean_acc.item(), input.size(0))
        AvgFF.update(average_f1.item(), input.size(0))
        ...
        if i % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'AVGF1{average_f1.val} ({average_f1.avg}\t'.format(
                   epoch, i, len(train_loader), batch_time=batch_time,
                   data_time=data_time, loss=losses, top1=top1, average_f1=AvgFF))
        ...
        if i % args.print_freq == 0:
            print('Test: [{0}/{1}]\t'
                  ...
                  'AVGF1{average_f1.val} ({average_f1.avg}\t'.format(
                   ...
                   top1=top1, average_f1=AvgFF))

    print(' * Prec@1 {top1.avg}'
          .format(top1=top1))
    # print('our results:',(top1.avg + AvgFF.avg)/2)

    return (top1.avg + AvgFF.avg)/2
